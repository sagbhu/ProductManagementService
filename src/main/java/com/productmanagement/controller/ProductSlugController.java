...
public class ProductSlugController {
	...
	public ResponseEntity<ProductSlug> createProductSlug(@RequestBody ProductSlug productSlug) {
		...
		ProductSlug newProductSlug = null;
		if (!productSlug.getSlugName().isEmpty()) {
			newProductSlug = productSlugService.createProductSlug(productSlug);
			logger.debug("createProductSlug method start in ProductSlugController");
			return new ResponseEntity<>(newProductSlug, HttpStatus.CREATED);
		}
		return new ResponseEntity<>(newProductSlug, HttpStatus.BAD_REQUEST);
	}
	...
	public ResponseEntity<ProductSlug> updateProductSlug(@RequestBody ProductSlugRequest productSlugRequest) {
		logger.debug("updateProductSlug method start in ProductSlugController");
		ProductSlug updatedProductSlug = productSlugService.updateProductSlug(productSlugRequest);
	...
	@GetMapping("/productSlugSearch/{name}")
	public ResponseEntity<List<ProductSlug>> getProductSlugByName(@PathVariable("name") String name) {
		logger.debug("getProductSlugById method start in ProductSlugController");
		List<ProductSlug> productSlug = productSlugService.getProductSlugByName(name);
		logger.debug("getProductSlugById method end in ProductSlugController");
		return new ResponseEntity<>(productSlug, HttpStatus.OK);
	}

	@PostMapping("/productSlugListByState")
	public ResponseEntity<List<ProductSlug>> getProductSlugByState(@RequestBody ProductSlug productSlug) {
		logger.debug("getAllProductSlugs method start in ProductSlugController");
		List<ProductSlug> productSlugList = productSlugService.getProductSlugByStateCategory(productSlug);
		logger.debug("getAllProductSlugs method end  in ProductSlugController");
		return new ResponseEntity<>(productSlugList, HttpStatus.OK);
	}
